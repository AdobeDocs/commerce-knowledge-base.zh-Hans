...
MDVA-35569修补程序解决了在购物车中指定状态后FPT（固定产品税）未显示在GraphQL中的问题。 安装[Quality Patches Tool (QPT)](/help/announcements/adobe-commerce-announcements/magento-quality-patches-released-new-tool-to-self-serve-quality-patches.md) 1.0.19后，即可使用此修补程序。 修补程序ID为MDVA-35569。 请注意，Adobe Commerce版本2.4.3中已修复此问题。
...
1. 启用FPT。
1. 创建属性（示例： *weee\_tax*）。
1. 创建添加了&#x200B;*weee\_tax*&#x200B;属性的测试产品（示例：*weetax*）。
1. 将FPT分配给&#x200B;*weee\_tax*&#x200B;属性上的California或其他州。
1. 在GraphQL中创建客户。
1. 在GraphQL中创建购物车。
1. 使用GraphQL将&#x200B;*weetax*&#x200B;产品添加到购物车。
1. 查询购物车：
