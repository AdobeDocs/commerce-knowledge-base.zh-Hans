...
description: 本文讨论了Elasticsearch插件生成的跟踪索引导致的内存问题。
...
本文讨论了Elasticsearch插件生成的跟踪索引导致的内存问题。
...
如果安装了ElasticSuite第三方插件，则可能会遇到Elasticsearch内存问题，并且Elasticsearch服务可能会因ElasticSuite跟踪索引而崩溃。 症状包括：

* Elasticsearch崩溃，没有内存错误。
...
* *“在Elasticsearch部署或记录错误时，您的群集中未发现活动节点”*。
* *在部署或日志错误中拒绝到[&lt;\*>_tracking_ log_event_&lt;\*>]“*”的映射更新。

## 原因

ElasticSuite具有创建跟踪索引的新功能。 这些跟踪索引记录哪些搜索词使用最多、哪些词产生最多成交额，以及哪些词导致无结果页面，以便商家可以创建同义词来修复它们。 它似乎不会删除跟踪索引，因此Elasticsearch资源不足并崩溃。
