...
1. 获取MySQL数据库凭据： `echo $MAGENTO_CLOUD_RELATIONSHIPS | base64 -d | json_pp`
1. 使用`mysql`连接到数据库： `mysql -hdatabase.internal -uuser -ppassword main`
1. 选择`main`数据库： `use main`
1. 查找所有正在运行的[!DNL cron]作业： `SELECT * FROM cron_schedule WHERE status = 'running';`
1. 复制运行时间超过正常时间的任何作业的`job_code`。
1. 使用上一步骤中的计划ID解锁特定[!DNL cron]作业： `./vendor/bin/ece-tools cron:unlock --job-code=<job_code_1> [... --job-code=<job_code_x>]`
...
1. 如果您看到长时间运行的[!DNL cron]作业可能属于块部署进程，则可以使用`kill`命令终止该进程。您可以识别&#x200B;**进程ID**（找到`PID`列），然后将该`PID`放入命令中以终止该进程。
**终止进程**&#x200B;命令为：
