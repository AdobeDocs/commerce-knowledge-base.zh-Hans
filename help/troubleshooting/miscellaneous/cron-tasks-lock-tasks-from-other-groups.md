...
>[!NOTE]
>
>无需将存在多个[!DNL cron]的旧`cron:run`配置传输到新[!DNL cron]计划；如上所述添加的常规`cron:run`任务已经足够。 但是，如果您有任何自定义作业，则需要转移这些作业。
...
* 如果您无法看到任务并收到[!DNL cron]“不允许使用此程序”*错误消息，则未启用自托管*。
